...
public class CsPanelInventory : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    bool isDrag = false, isEmpty = false, isPutOn = false;
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        if (!isDrag)
        {
            ...
            else if (eventData.pointerCurrentRaycast.gameObject.CompareTag("Weapon") ||
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        if (selectedOB != null)
            ...
            {
                if (result[i].gameObject.transform.parent.tag == "HasChild" && result[i].gameObject != selectedOB)
                {
                ...
                else if (result[i].gameObject.transform.tag == "Slot")
                {
                    if (isEmpty)
                    {
                        isEmpty = false;
                ...
                else if (result[i].gameObject.transform.tag == "EquipmentEmpty")
                {
                    if (selectedOB.tag == "Helmet" && result[i].gameObject.name == "helmet" ||
                        selectedOB.tag == "Cloth" && result[i].gameObject.name == "cloth" ||
                        selectedOB.tag == "Pants" && result[i].gameObject.name == "pants" ||
                        selectedOB.tag == "Weapon" && result[i].gameObject.name == "left" ||
                        selectedOB.tag == "Weapon" && result[i].gameObject.name == "right"
                        )
                    {
                        GameObject tempParent = selectedOB.transform.parent.gameObject;
                ...
                else if (result[i].gameObject.transform.tag == "EquipmentFull")
                {
                    if (selectedOB.tag == "Helmet" && result[i].gameObject.name == "helmet" ||
                        selectedOB.tag == "Cloth" && result[i].gameObject.name == "cloth" ||
                        selectedOB.tag == "Pants" && result[i].gameObject.name == "pants" ||
                    ...
                    {
                        Transform tempParent = selectedOB.transform.parent.gameObject.transform;

                        selectedOB.transform.SetParent(result[i].gameObject.transform);
                        selectedOB.transform.position = result[i].gameObject.transform.position;

                        Transform tempTarget = result[i].gameObject.transform.GetChild(0).transform;
                        result[i].gameObject.transform.GetChild(0).SetParent(tempParent);
                        tempTarget.transform.position = tempParent.position;
